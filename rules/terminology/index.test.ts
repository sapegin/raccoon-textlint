...
describe(getTerms, () => {
  ...
  test('should load user terms from a file', () => {
    const result = getTerms(false, './test/terms.jsonc', []);
    expect(result).not.toContain('Airbnb');
...
describe(getMultipleWordRegExp, () => {
  ...
  test('should match several variants', () => {
    ...
    expect(result2).toContain('webpack');
  });
});
...
describe(getExactMatchRegExp, () => {
  test('returned RegExp should match exact term', () => {
    ...
    expect(regexp.test('Webpack')).toBe(true);
  });
  ...
  test('returned RegExp should not match in the middle of the word', () => {
    const regexp = new RegExp(getExactMatchRegExp('webpack'), 'igm');
    expect(regexp.test(`FooWebpack`)).toBe(false);
    expect(regexp.test(`WebpackFoo`)).toBe(false);
    expect(regexp.test(`FooWebpackFoo`)).toBe(false);
  });
});
...
describe(getAdvancedRegExp, () => {
  test('should return an exact match regexp', () => {
    ...
    expect(regexp.test('bug-fix')).toBe(true);
  });
  ...
  test('should return regexp as is if it has look behinds', () => {
    ...
    expect(regexp.test('use Base64')).toBe(true);
  });
  ...
  test('should return regexp as is if it has positive look ahead', () => {
    ...
    expect(regexp.test('Base64 foo')).toBe(true);
  });
  ...
  test('should return regexp as is if it has negative look ahead', () => {
    ...
    expect(regexp.test('Base64')).toBe(true);
    expect(regexp.test('Base64 foo')).toBe(false);
  });
  ...
  test('should not match words inside filenames', () => {
    ...
    expect(regexp.test('typings.for.css.modules.loader')).toBe(false);
    expect(regexp.test('typings-for-css-modules-loader')).toBe(false);
    expect(regexp.test('typings_for_css_modules_loader')).toBe(false);
    expect(regexp.test('typings for css modules loader')).toBe(true);
  });
});
